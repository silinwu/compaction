...
void Setup(LBM::Domain &dom, void *UD)
{
    ...
    for(size_t ix=0; ix<nx; ++ix)
    {
        ...
        double* f2 = dom.F[ix][ny-1][0];
        double* f3 = dom.F[ix][ny-2][0];
        ...
        Vec3_t idx1(ix,0,0);
        Vec3_t idx2(ix,ny-1,0);
        dom.CalcPropsForCell(idx1);
        dom.CalcPropsForCell(idx2);
    }
...
int main (int argc, char **argv) try
{
    ...
    if(!iscontinue)
    {
        ...
        }//pressure plane the tag is very important!!! keep the same with the solve function
        //move
    ...
    for(size_t ip=0; ip<dom.Particles.size(); ++ip)
    {
        ...
        if(dom.Particles[ip].Tag == 0)
        {
            ...
            if(dom.Particles[ip].Tag == -1)
            {
                dom.Particles[ip].Ff =  0.0, M_PI*dom.Particles[ip].R*dom.Particles[ip].R*(rhos/rho-1)*my_dat.g,0.0;//plane pressure force
            }
            if(dom.Particles[ip].Tag == -2)
            {
                dom.Particles[ip].Ff =  0.0, -M_PI*dom.Particles[ip].R*dom.Particles[ip].R*(rhos/rho-1)*my_dat.g,0.0;//plane pressure force
            }
        }
    ...
    //solving
    dom.SolveIBM1( Tf, dtout, "test_wu_3", Setup, id/*index for moving with restriction*/, 0/*restriction direrction*/, NULL);
