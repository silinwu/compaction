...
inline void Domain::MoveParticlesWithRestriction(std::vector<int> &id, size_t xy)
{
    int iN = id.size();
    std::vector<Vec3_t> Fv;
    ...
    for (size_t i=0; i<Particles.size(); i++)
    {
        ...
        {
            if(Particles[i].Tag!=id[in]) continue;
            // std::cout<<1<<std::endl;
            Fv[in] += Particles[i].Fh + Particles[i].Fc;
            Mv[in] += Particles[i].M;
        }
    ...
    for (size_t i=0; i<Particles.size(); i++)
    {
        ...
        {
            Fv[in](xy) = 0;
            if(Particles[i].Tag==id[in])
            {
                Particles[i].Fc = Fv[in]/Mv[in]*Particles[i].M;
